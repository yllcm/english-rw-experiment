"""
主程序入口 - 爬取 OpenAlex 数据并计算论文颠覆性指数
...
from disruptiveness import DisruptivenessCalculator
...
def save_results(results: list, filepath: str):
    ...
    Args:
        ...
        filepath: 输出文件路径
    """
    # 对结果进行预处理：将列表/字典等复杂类型转为可读字符串
    processed = []
    for r in results:
        row = {}
        for key, value in r.items():
            if isinstance(value, list):
                # 列表转为逗号分隔字符串（如 [1990, 1992] -> "1990, 1992"）
    ...
    # 只保留 column_order 中实际存在的列
    existing_columns = [col for col in column_order if col in df.columns]
    # 补充 column_order 中未定义的额外列（放在最后）
...
def visualize_results(df: pd.DataFrame, filepath: str):
    ...
    if has_dv:
        ...
        # 5. Rela_Dz vs Discipline Variety 散点图
        ax5 = axes[1, 1]
        sns.scatterplot(
...
def main():
    ...
    # 第二步：计算颠覆性指数
    print(f"\n[步骤 2/3] 计算颠覆性指数 (共 {len(works)} 篇论文)...")
    calculator = DisruptivenessCalculator()
    results = calculator.calculate_batch(works)
